...
export default function ImageUpload(props: ImageUploadProps) {
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>) => {
    ...
    else toast.error('invalid Image Uploaded')
  }
  ...
  return (
    <Stack gap={.5} sx={style.root}>
      <Stack
        sx={{ ...style.box, borderColor: (theme) => helperText ? `${theme.palette.error.main} !important` : `${theme.palette.primary.light}` }}
        ...
        {file ?
          ...
          <Stack sx={style.uploadBox}>
            ...
            <Typography variant='body2'>
              {isMdDown ? 'uploadImage' : dragOver ? 'dropHere' : 'dropImage'}
            </Typography>
          </Stack>
        }
      </Stack>
